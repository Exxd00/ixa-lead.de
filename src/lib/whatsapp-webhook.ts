...
export const whatsappWebhookSchemaVersion = 2 as const;
export const maxWhatsAppWebhookBytes = 256 * 1024;
export const maxWhatsAppMessagesPerWebhook = 50;

export class WhatsAppMessageLimitError extends Error {
  constructor() {
    super("whatsapp_message_limit_exceeded");
    this.name = "WhatsAppMessageLimitError";
  }
}
...
function extractReferenceHashes(messageText: string): string[] {
  ...
  return Array.from(candidates, (candidate) => sha256Hex(candidate)).slice(
    0,
    8,
  );
}
  ...
  for (const entryValue of array(root.entry)) {
    ...
    for (const changeValue of array(entry?.changes)) {
      ...
      for (const messageValue of array(value?.messages)) {
        ...
        const messageText = extractMessageText(message, messageType).slice(
          0,
          4000,
        );
        parsed.push({
        ...
        if (parsed.length > maxWhatsAppMessagesPerWebhook) {
          throw new WhatsAppMessageLimitError();
        }
      }
    }
  }
